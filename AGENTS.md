...
- Prioritise the counts from the latest log snapshot (higher first): `foldLeft`, `distinct`, `single`, `filter`, `sift`, `merge`, `spread`, `shuffle`, `reverse`, `assert`, then the URL/base64 helpers.
## Execution Rules
