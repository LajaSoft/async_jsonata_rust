...
pub fn create_builtin_registry() -> HashMap<String, JsonFunction> {
    ...
    registry.insert(
        "abs".to_string(),
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn("abs", Some(1), |args| {
    ...
    registry.insert(
        "floor".to_string(),
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn(
            "floor",
            Some(1),
            |args| {
                let num = json_value_to_number(&args[0]);
    ...
    registry.insert(
        ...
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn(
            ...
            |args| {
                let num = json_value_to_number(&args[0]);
                let precision =
                    json_value_to_number(&args.get(1).unwrap_or(&JsonValue::Number(0.0)));
    ...
    registry.insert(
        ...
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn(
            "boolean",
            Some(1),
            |args| Ok(core::boolean(&args[0])),
        ))),
    );

    registry.insert(
        "not".to_string(),
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn("not", Some(1), |args| {
            Ok(core::not(&args[0]))
        }))),
    );
    ...
    registry.insert(
        ...
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn(
            "keys",
            Some(1),
            |args| Ok(core::keys(&args[0])),
        ))),
    );

    registry.insert(
        "zip".to_string(),
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn("zip", None, |args| {
            Ok(core::zip(args))
        }))),
    );
    ...
    registry.insert(
        ...
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn(
            "append",
            Some(2),
            |args| Ok(core::append(&args[0], &args[1])),
        ))),
    );
    ...
    registry.insert(
        "single".to_string(),
        JsonFunction::new(Arc::new(BuiltinCallable::async_fn(
            "single",
            Some(2),
            |ctx, args| {
    ...
    registry.insert(
        ...
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn(
            "substringBefore",
            Some(2),
            |args| strings::substring_before(&args[0], &args[1]),
        ))),
    );
    ...
    registry.insert(
        ...
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn(
            "substringAfter",
            Some(2),
            |args| strings::substring_after(&args[0], &args[1]),
        ))),
    );
    ...
    registry.insert(
        ...
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn(
            "trim",
            Some(1),
            |args| strings::trim(&args[0]),
        ))),
    );

    registry.insert(
        "pad".to_string(),
        JsonFunction::new(Arc::new(BuiltinCallable::sync_fn("pad", Some(3), |args| {
